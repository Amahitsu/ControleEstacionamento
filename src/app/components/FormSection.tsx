...
const FormSection: React.FC = () => {
  ...
  const calcularValorTotal = (
    ...
    const valorTarifa =
      tarifas.find((t) => t.tipoVeiculoId === idTipoVeiculo)?.valor ?? 0;
    if (valorTarifa <= 0) {
    ...
    const diferencaEmHoras = moment
      .duration(dataSaida.diff(dataEntrada))
      .asHours();
  ...
  const handleLiberarCupom = () => {
    if (resultado) {
      const valorCalculado = calcularValorTotal(
        resultado.dataHoraEntrada,
        null,
        resultado.idTipoVeiculo
      );
    ...
    <div className={styles.formSection}>
      ...
      {resultado && (
        <div className={styles.resultado}>
          ...
          <p>
            <strong>Tipo de Veículo:</strong> {resultado.tipoVeiculo}
          </p>
          <p>
            <strong>Data e Hora de Entrada:</strong>{" "}
            {new Date(resultado.dataHoraEntrada).toLocaleString()}
          </p>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-2/5">
            <h2 className="text-lg font-bold mb-4">
              Cupom {cupomSelecionado.id}
            </h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <p>
                <strong>Placa:</strong> {cupomSelecionado.placa}
              </p>
              <p>
                <strong>Valor Total: R$ </strong> {valorTotal.toFixed(2)}
              </p>
              <p>
                <strong>Hora Entrada:</strong>{" "}
                {formatarDataMoment(cupomSelecionado.dataHoraEntrada)}
              </p>
              <p>
                <strong>Hora Saída:</strong> {obterHorarioLocal()}
              </p>
            </div>
